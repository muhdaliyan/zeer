    ...
    async def send_message(self, message: str, context: ChatContext) -> Response:
        ...
        try:
            ...
            if hasattr(response, 'prompt_eval_count') or hasattr(response, 'eval_count'):
                usage = {
                    "promptTokens": getattr(response, 'prompt_eval_count', 0),
                    "completionTokens": getattr(response, 'eval_count', 0),
                    "totalTokens": getattr(response, 'prompt_eval_count', 0) + getattr(response, 'eval_count', 0)
                }
